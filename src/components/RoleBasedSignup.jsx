// Full updated component with backend error popups and dropdown support
...
const jobseekerSteps = [
  { id: "first_name", label: "Enter your first name", type: "text", validation: /^[A-Za-z]{2,}$/, path: "auth.signup.job_seeker.first_name" },
  { id: "last_name", label: "Enter your last name", type: "text", validation: /^[A-Za-z]{2,}$/, path: "auth.signup.job_seeker.last_name" },
  { id: "email", label: "Enter your email", type: "email", validation: /^[^\s@]+@[^\s@]+\.[^\s@]+$/, path: "auth.signup.job_seeker.email" },
  { id: "password", label: "Create a password", type: "password", validation: /^.{8,}$/, path: "auth.signup.job_seeker.password" },
  { id: "phone_number", label: "Enter your phone number", type: "tel", validation: /^\+?\d{10,14}$/, path: "auth.signup.job_seeker.phone_number" },
];

const employerSteps = [
  { id: "company_name", label: "Enter your company name", type: "text", validation: /^.{2,}$/, path: "auth.signup.employer.company_name" },
  { id: "first_name", label: "Enter contact person's first name", type: "text", validation: /^[A-Za-z]{2,}$/, path: "auth.signup.employer.first_name" },
  { id: "last_name", label: "Enter contact person's last name", type: "text", validation: /^[A-Za-z]{2,}$/, path: "auth.signup.employer.last_name" },
  { id: "email", label: "Company email address", type: "email", validation: /^[^\s@]+@[^\s@]+\.[^\s@]+$/, path: "auth.signup.employer.email" },
  { id: "password", label: "Create a secure password", type: "password", validation: /^.{8,}$/, path: "auth.signup.employer.password" },
  { id: "company_website", label: "Enter your company website", type: "url", validation: /^(https?:\/\/)?[\w\-]+(\.[\w\-]+)+[/#?]?.*$/, path: "auth.signup.employer.company_website" },
  { id: "company_size", label: "Select your company size", type: "select", options: ["1-10", "11-50", "51-200", "200+"], validation: /^.+$/, path: "auth.signup.employer.company_size" },
  { id: "phone_number", label: "Enter your contact phone number", type: "tel", validation: /^\+?[0-9]{10,15}$/, path: "auth.signup.employer.phone_number" },
  { id: "role_in_company", label: "Your role in the company", type: "text", validation: /^.{2,}$/, path: "auth.signup.employer.role_in_company" },
];
...
export default function RoleBasedSignup({ role }) {
  ...
  useEffect(() => {
    if (audioData?.audio_url && audioPlaying) {
      ...
      const playAudio = async () => {
        try { await audio.play(); } catch (err) { console.error("Audio play error:", err); }
      };
      playAudio();
  ...
  const onSubmit = async () => {
    try {
      ...
      const res = role === "jobseeker" ? await dispatch(jobSeekerSignup(payload)) : await dispatch(employerSignup(payload));

      if ((role === "jobseeker" && jobSeekerSignup.fulfilled.match(res)) ||
    ...
    <div className="min-h-screen bg-gradient-to-br from-gray-900 to-gray-800 text-white flex items-center justify-center p-4 pt-20 sm:pt-0">
      <div className="max-w-md w-full space-y-4">
        <div className="flex justify-between items-center">
          <h2 className="text-teal-400 font-bold text-xl">{role === "employer" ? "Employer Signup" : "Jobseeker Signup"}</h2>
          <select
            value={language}
