...
export const getTTSAudio = createAsyncThunk(
  ...
  async ({ promptPath, language }, { rejectWithValue }) => {
    try {
      const res = await apiInstance.get(`/tts/audio`, {
        params: { prompt_path: promptPath, language },
      });
      return res.data; // { audio_url, language, prompt_path }
    } catch (err) {
      return rejectWithValue(err.response?.data?.message || "Failed to fetch TTS audio.");
    }
  }
);
...
export const getAvailablePrompts = createAsyncThunk(
  ...
  async (_, { rejectWithValue }) => {
    try {
      const res = await apiInstance.get(`/tts/prompts`);
      return res.data; // Array of prompt strings
    } catch (err) {
...
export const getAvailableLanguages = createAsyncThunk(
  ...
  async (_, { rejectWithValue }) => {
    try {
      const res = await apiInstance.get(`/tts/languages`);
      return res.data; // Array of languages like ["en", "hi", ...]
    } catch (err) {
