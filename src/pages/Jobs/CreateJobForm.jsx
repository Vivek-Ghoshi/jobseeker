...
const CreateJobForm = () => {
  ...
  // Autofill form fields from jobDetails
  useEffect(() => {
    if (jobDetails) {
      if (jobDetails.jobtitle) setValue('title', jobDetails.jobtitle);
      if (jobDetails.location) setValue('location', jobDetails.location);
      if (jobDetails.job_type) setValue('job_type', jobDetails.job_type.toLowerCase().replace(/-/g, '_'));
      if (jobDetails.salary_min) setValue('salary_min', jobDetails.salary_min);
      if (jobDetails.salary_max) setValue('salary_max', jobDetails.salary_max);
      if (jobDetails.job_description) setValue('description', jobDetails.job_description);
      if (Array.isArray(jobDetails.requirements)) {
        setValue('requirements', jobDetails.requirements.join(', '));
      }
      if (Array.isArray(jobDetails.responsibilities)) {
        setValue('responsibilities', jobDetails.responsibilities.join(', '));
      }
    }
  }, [jobDetails, setValue]);
  ...
  return (
    <div className="min-h-screen bg-gradient-to-br from-black via-gray-900 to-[#0f0f1a] flex items-center justify-center p-6">
      <form
        onSubmit={handleSubmit(onSubmit)}
        className="bg-gradient-to-bl from-black via-gray-900 to-[#0f0f1a] text-white w-full max-w-5xl rounded-xl shadow-2xl p-8 md:p-12 grid grid-cols-1 md:grid-cols-2 gap-8 animate-fadeIn"
      >
        ...
        <div className="md:col-span-2">
          ...
          <textarea
            ...
            className="custom-scrollbar w-full bg-[#334155] text-sm px-3 py-2 rounded-md outline-none border border-transparent focus:border-cyan-400 transition resize-none"
