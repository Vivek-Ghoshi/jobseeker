...
const ProctoringDemo = () => {
  ...
  const [connectionStatusText, setConnectionStatusText] =
    useState("Not Connected");
  const [connectionStatusClass, setConnectionStatusClass] = useState(
  ...
  useEffect(() => {
    const handleVisibilityChange = () => {
      if (document.hidden) {
        ...
        alert("Tab switch detected. Exam terminated.");
  ...
  const updateProctoringStatus = useCallback(
    ({ face, neck, eye, lip, overall, details }) => {
      setFaceStatus(face);
      setNeckStatus(neck);
      setEyeStatus(eye);
  ...
  const updateAlerts = useCallback(() => {
    ...
    if (
      alertMessages.includes("Eyes closed or suspicious eye activity detected!")
    ) {
      eye = "ALERT";
      overall = "ALERT";
      details += "Eyes closed or suspicious eye activity detected!\n";
    }
    updateProctoringStatus({ face, neck, eye, overall, details });

    if (
      alertMessages.length > 0 &&
      !logs.some(
        (log) => log.message === alertMessages[alertMessages.length - 1]
      )
    ) {
  ...
  const startProctoring = async () => {
    ...
    wsRef.current.onmessage = (event) => {
      ...
      if (faceCount === 0) {
        if (!noFaceDetectedSinceRef.current)
          noFaceDetectedSinceRef.current = now;
  ...
  const StatusItem = ({ label, status }) => {
    const bgColor =
      status === "OK"
        ...
        : "bg-red-800/20 border-red-700 animate-pulse";
    const iconColor = status === "OK" ? "bg-green-500" : "bg-red-500";
    const textColor = status === "OK" ? "text-green-300" : "text-red-300";

    return (
      <div
        className={`flex items-center p-4 rounded-lg shadow-inner border ${bgColor}`}
      >
    ...
    <div className="min-h-screen bg-gradient-to-br from-gray-950 via-gray-900 to-gray-800 text-gray-100 p-2 sm:p-4">
      <div className="max-w-7xl mx-auto grid grid-cols-1 lg:grid-cols-3 gap-4">
        ...
        <div className="col-span-1 lg:col-span-3 flex justify-between items-center px-4 py-2 rounded-xl bg-gray-800/60 backdrop-blur-md border border-gray-700 shadow-md">
          ...
          <div
            className={`px-4 py-1 rounded-full font-bold text-sm sm:text-base shadow transition-all duration-300 ${connectionStatusClass}`}
          >
        ...
        <div
          className={`col-span-1 lg:col-span-3 text-center font-extrabold text-xl sm:text-2xl mt-2 px-3 py-2 rounded-lg border shadow-md transition-all duration-300
                ${
                  overallStatus === "OK"
                    ? "bg-green-800/20 text-green-300 border-green-700"
                    : "bg-red-800/20 text-red-300 border-red-700 animate-pulse"
                }`}
        >
        ...
        <div className="col-span-1 lg:col-span-3 bg-gray-900/50 rounded-xl border border-gray-700 p-3 mt-2 shadow-inner h-40 overflow-y-auto text-xs sm:text-sm scrollbar-thin scrollbar-thumb-gray-600 scrollbar-track-gray-800">
          <h2 className="text-center font-bold text-blue-300 mb-1">
            System Logs
          </h2>
          {logs.map((log, index) => (
            <div key={index} className="flex text-xs mb-1">
              <span className="text-green-500 font-mono mr-2 flex-shrink-0">
                {log.time}
              </span>
              <span
                className={`font-mono ${
          ...
          {logs.length === 0 && (
            <p className="text-gray-500 text-center">No recent logs.</p>
          )}
        </div>
      </div>
    </div>
